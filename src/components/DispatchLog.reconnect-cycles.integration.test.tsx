...
describe("DispatchLog across repeated disconnect/reconnect cycles", () => {
  it("never duplicates rows across five outage/replay cycles", async () => {
    renderLog();
    ...
    for (let cycle = 0; cycle < 5; cycle += 1) {
      ...
      socket.drop();
      const buffered = [liveRow(TYPE_A), liveRow(TYPE_B)];
      for (const row of buffered) socket.push(row);
      seen.push(...buffered);

      // Reconnect replays the entire backlog from every previous cycle too.
      socket.reconnect();
      await waitForLoaded(PAGE + seen.length);
      await settle();
  ...
  it("stays deduped when repeated cycles interleave with pagination", async () => {
    ...
    for (let cycle = 0; cycle < 3; cycle += 1) {
      socket.drop();
      const row = liveRow(cycle % 2 === 0 ? TYPE_B : TYPE_A);
      socket.push(row);
      seen.push(row);
      socket.reconnect();
      await settle();
      ...
      // Replay once more after the page landed.
      socket.reconnect();
      await settle();

      for (const r of seen) {
        expect(rowIds().filter((id) => id === r.id).length).toBe(1);
      }
      expectNoDuplicates();
      expectNewestFirst();
      expect(loadedCount()).toBe(rowsOnScreen().length);
  ...
  it("stays deduped when repeated cycles interleave with manual refreshes", async () => {
    const user = userEvent.setup();
    renderLog();
    await waitForLoaded(PAGE);
    await waitFor(() => expect(socket.handlers.length).toBeGreaterThan(0));
    ...
    for (let cycle = 0; cycle < 4; cycle += 1) {
      socket.drop();
      const missed = liveRow(cycle % 2 === 0 ? TYPE_A : TYPE_B);
      socket.push(missed);
      seen.push(missed);

      // A refresh reads the missed row straight from the database…
      await user.click(refresh);
      await settle();
      // …then the socket comes back and replays the same insert.
      socket.reconnect();
      await settle();
